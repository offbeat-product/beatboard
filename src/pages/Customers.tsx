...
const Customers = () => {
  ...
  if (d.isLoading) {
    return <div className="flex items-center justify-center h-64"><span className="text-muted-foreground text-sm">読み込み中...</span></div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ConcentrationCard
          ...
          target={d.targetTop1}
          isOver={isTop1Over}
          targetLabel={`目標 ${d.targetTop1}%以下`}
        />
        <ConcentrationCard
          label="上位3社集中度"
          value={d.top3Pct}
          target={d.targetTop3}
          isOver={isTop3Over}
          targetLabel={`目標 ${d.targetTop3}%以下`}
        />
      </div>
      ...
      {/* Client Table */}
      <div className="bg-card rounded-lg shadow-sm animate-fade-in" style={{ animationDelay: "300ms" }}>
        <Table>
          <TableHeader>
  ...
  return (
    <div className="bg-card rounded-lg shadow-sm p-5 animate-fade-in">
      ...
      <div className="flex items-baseline gap-2 mb-1">
        <span className={`text-2xl font-bold font-mono-num ${isOver ? "text-destructive" : "text-chart-green"}`}>
          {value.toFixed(1)}%
        </span>
      ...
      <div className="relative h-3 w-full rounded-full bg-secondary overflow-hidden">
        ...
        {/* Target marker */}
        <div
          className="absolute inset-y-0 w-0.5 bg-chart-green"
          style={{ left: `${target}%` }}
        />
      </div>
    </div>
